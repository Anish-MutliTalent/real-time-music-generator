...
import { generateRandomInitialPrompts, getRandomSuggestions, COLORS } from './constants';
...
export default function App() {
  ...
  const [mode, setMode] = useState<'prompt' | 'preset' | 'export'>('prompt');
  ...
  const updateApiPrompts = useCallback(
    async (currentPrompts: WeightedPrompt[]) => {
      if (isPlaying) {
  ...
  const handleAddPreset = (text: string, color?: string) => {
    const np = {
  ...
  // If in export mode, show only the export panel (hide all other UI)
  if (mode === 'export') {
    return (
      <div className="h-screen w-full flex items-center justify-center bg-black">
        <div className="absolute top-6 right-6 z-50">
          <button onClick={() => setMode('prompt')} className="px-4 py-2 rounded-full bg-neutral-900 text-white">Close Export</button>
        </div>
        <ExportPanel />
      </div>
    );
    ...
    <div className="h-screen w-full flex flex-col bg-black overflow-hidden font-sans">
      <div className="flex-1 overflow-y-auto px-6 pt-6 pb-40">
        <div className="w-full mx-auto">
          ...
          <div className="absolute top-6 right-6 z-50">
            <div className="bg-[#111]/90 backdrop-blur-xl rounded-full p-1 flex items-center gap-1 border border-white/10 shadow-lg">
              <button
                onClick={() => setMode('prompt')}
                className={`px-4 py-2 rounded-full ${mode === 'prompt' ? 'bg-[#ff9cd6] text-black' : 'text-neutral-300'}`}>
                Prompt
              </button>
              <button
                onClick={() => setMode('preset')}
                className={`px-4 py-2 rounded-full ${mode === 'preset' ? 'bg-[#ff9cd6] text-black' : 'text-neutral-300'}`}>
                Presets
              </button>
              <button
                onClick={async () => {
                  if (isPlaying) {
                    const ok = window.confirm('Switching to Export will stop playback. Continue?');
                    if (!ok) return;
                    try {
                      await musicService.pause();
                    } catch (e) {}
                ...
                className={`px-4 py-2 rounded-full ${mode === 'export' ? 'bg-[#ff9cd6] text-black' : 'text-neutral-300'}`}>
                Export
              </button>
            </div>
          </div>
          ...
          {mode === 'prompt' ? (
                ...
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' && textInput) {
                      const np = { id: Date.now().toString(), text: textInput, weight: 0.5, color: '#cba6f7', isActive: true } as WeightedPrompt;
                      const up = [np, ...prompts];
                      setPrompts(up);
                      setTextInput('');
                      updateApiPrompts(up);
              ...
              <div className="w-full max-w-3xl flex flex-wrap gap-2 justify-center mb-12">
                {suggestions.map((s) => (
                  <button
                    key={s}
                    onClick={() => {
                      const np = { id: Date.now().toString(), text: s, weight: 0.5, color: '#f9e2af', isActive: true } as WeightedPrompt;
                      const up = [np, ...prompts];
                      setPrompts(up);
                      updateApiPrompts(up);
                    }}
            ...
            <div className="flex gap-6 items-start justify-center w-full">
              ...
              <div className="w-[22rem]">
                <PresetPanel onAddPreset={handleAddPreset} />
              </div>
            </div>
          )}
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 w-full max-w-5xl px-6 z-50">
        <div className="bg-[#111]/90 backdrop-blur-xl rounded-[2.5rem] p-3 flex items-center gap-4 border border-white/10 shadow-2xl">
          <button
            onClick={togglePlay}
            className={`w-14 h-14 rounded-full flex items-center justify-center transition-all ${isPlaying ? 'bg-[#ff9cd6] text-black' : 'bg-[#333] text-white'}`}>
            {isPlaying ? (
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              ...
              >
                ...
                <span className="text-xs font-bold text-white whitespace-nowrap overflow-hidden text-ellipsis max-w-[50px]">{macroState.key === 'auto' ? 'Auto' : (macroState.key as string).replace(' ', '')}</span>
              </button>
            </div>
          </div>
